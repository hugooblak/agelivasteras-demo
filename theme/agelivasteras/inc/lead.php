 ...
 * or photo. They call ag_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional AG-EL i Västerås AB.
 *
 ...
 * @return string
 */
function ag_sample_note() {
	return ag_lead( 'ar_riktig' ) ? '' : ' <span class="ag-sample">(platshållare)</span>';
}
 ...
 * @return string File name for ag_lead_img().
 */
function ag_lead_bild( $i ) {
	$bilder = (array) ag_lead( 'bilder', array() );
 ...
 * @return string
 */
function ag_ring_label() {
	$namn = ag_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, ag_lead( 'telefon' ) ) : 'Ring ' . ag_lead( 'telefon' );
}
