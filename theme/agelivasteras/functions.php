<?php
/**
 * AG-EL i Västerås AB theme functions.
 *
 * Kept short on purpose. Design lives in theme.json, markup in templates and patterns.
 * Lead capture lives in the "AG-EL i Västerås AB Leads" plugin, so leads survive a theme change.
 *
 ...
 * Patterns read these through ag_lead() so one demo can be rebuilt for any firm.
 */
...
add_action(
	'wp_enqueue_scripts',
	function () {
		wp_enqueue_style( 'agelivasteras', get_stylesheet_uri(), array(), wp_get_theme()->get( 'Version' ) );
	}
);
add_action(
	'after_setup_theme',
...
add_action(
	'init',
	function () {
		register_block_style( 'core/paragraph', array( 'name' => 'eyebrow', 'label' => __( 'Eyebrow label', 'agelivasteras' ) ) );
		register_block_style( 'core/list', array( 'name' => 'checklist', 'label' => __( 'Tick list', 'agelivasteras' ) ) );

		register_block_pattern_category( 'agelivasteras-sections', array( 'label' => __( 'AG-EL i Västerås AB sections', 'agelivasteras' ) ) );
		register_block_pattern_category( 'agelivasteras-pages', array( 'label' => __( 'AG-EL i Västerås AB page layouts', 'agelivasteras' ) ) );
	}
);
