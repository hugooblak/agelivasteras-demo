...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'agelivasteras-leads' ),
					'singular_name' => __( 'Lead', 'agelivasteras-leads' ),
					'edit_item'     => __( 'Förfrågan', 'agelivasteras-leads' ),
					'search_items'  => __( 'Search leads', 'agelivasteras-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'agelivasteras-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'agelivasteras-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'agelivasteras-leads' ),
			'npl_phone'    => __( 'Phone', 'agelivasteras-leads' ),
			'npl_service'  => __( 'Needs', 'agelivasteras-leads' ),
			'npl_timeline' => __( 'When', 'agelivasteras-leads' ),
			'npl_zip'      => __( 'ZIP', 'agelivasteras-leads' ),
			'npl_source'   => __( 'Source', 'agelivasteras-leads' ),
			'npl_status'   => __( 'Status', 'agelivasteras-leads' ),
			'date'         => __( 'Received', 'agelivasteras-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'agelivasteras-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'agelivasteras-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'agelivasteras-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'agelivasteras-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'agelivasteras-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'agelivasteras-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'agelivasteras-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'agelivasteras-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'agelivasteras-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'agelivasteras-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'agelivasteras-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'agelivasteras-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
