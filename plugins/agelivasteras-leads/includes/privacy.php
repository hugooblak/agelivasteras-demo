...
add_filter(
	...
	function ( $exporters ) {
		$exporters['agelivasteras-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'agelivasteras-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'agelivasteras-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'agelivasteras-leads',
						'group_label' => __( 'Quote requests', 'agelivasteras-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['agelivasteras-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'agelivasteras-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
